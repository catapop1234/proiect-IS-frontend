...
export default function ResultsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  useEffect(() => {
    const fetchPlaces = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE}/api/search/${type}`, {
          method: "POST",
  ...
  const loadMore = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/search/${type}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ city, radius, page_token: nextPageToken }),
      });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <h1 className="text-3xl font-bold mb-2">
              {TYPE_LABELS[type] || "Places"} in{" "}
              <span className="gradient-text">{city}</span>
            </h1>
            <p className="text-[var(--text-secondary)]">
              {places.length} results found within {radius} km
            </p>
          </motion.div>
                ...
                <div className="text-center mt-10">
                  <motion.button
                    onClick={() => {
                      ...
                      if (displayCount + 10 >= places.length && nextPageToken) {
                        loadMore();
                      }
                    }}
